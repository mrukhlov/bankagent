	...
	if req['result']['parameters'].has_key('date') and len(req['result']['parameters']['date']) > 0:
		...
		try:
			if req['result']['parameters'].has_key('date-period'):
				# req['result']['parameters']['date-period'] = [datetime.datetime.strptime(x, '%Y-%m-%d').strftime('%B %-d, %Y') for x in req['result']['parameters']['date-period'].split('/')]
				date_p = req['result']['parameters']['date-period'].split('/')
				# print date_p
				req['result']['parameters']['date-period'] = []
				for x in date_p:
					# if datetime.datetime.strptime(x, '%Y-%m-%d').month == datetime.datetime.today().month:
...
def test():
	return 'bank_agent Test is done!'
